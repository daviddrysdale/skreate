...
use super::{
    ...
    Centimetres, MoveId, SkatingMoveId,
};
use crate::{code, moves, params, Code, Edge, MoveParam, ParseError, PreTransition, TextPosition};
...
impl Rocker {
    ...
    pub fn from_params(
        ...
        let len1a = Centimetres(len1.0 * 85 / 100);
        let len1b = len1 - len1a;
        let angle1b = 80;
        let angle1a = angle1;

        let len2a = Centimetres(len2.0 * 75 / 100);
        let len2b = Centimetres(20);
