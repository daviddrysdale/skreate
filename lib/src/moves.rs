...
impl SkatingMoveId {
    ...
    pub fn info(&self) -> &'static Info {
        match self {
            ...
            Self::Flip(_count) => &jump::Flip::INFO,
            Self::Lutz(_count) => &jump::Lutz::INFO,
    ...
    pub(crate) fn construct(
        ...
        macro_rules! make_move {
            { $module:ident :: $variant:ident } => {
                Box::new($module::$variant::from_params(
                    input,
                    text_pos,
                    pre_transition,
                    entry_code,
        ...
        Ok(match self {
            ...
            Self::ClosedMohawk => make_move!(mohawk::ClosedMohawk),
            Self::Bracket => make_move!(bracket::Bracket),
            Self::Rocker => make_move!(rocker::Rocker),
            Self::Counter => make_move!(counter::Counter),
            Self::OpenChoctaw => make_move!(choctaw::OpenChoctaw),
            Self::ClosedChoctaw => make_move!(choctaw::ClosedChoctaw),
            Self::ChangeOfEdge => make_move!(coe::ChangeOfEdge),
            Self::Twizzle(count) => make_move!(twizzle::Twizzle, *count),
            Self::Loop => make_move!(loopfig::Loop),
            Self::Hop => make_move!(hop::Hop),
            Self::Salchow(count) => make_move!(jump::Salchow, *count),
            Self::ToeLoop(count) => make_move!(jump::ToeLoop, *count),
            Self::LoopJump(count) => make_move!(jump::Loop, *count),
            Self::Flip(count) => make_move!(jump::Flip, *count),
            Self::Lutz(count) => make_move!(jump::Lutz, *count),
            Self::Axel(count) => make_move!(jump::Axel, *count),
        })
    }
}
